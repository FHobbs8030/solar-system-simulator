class SolarSystem {
  constructor() {
    ...
    this.drag = { active: false, x: 0, y: 0, pointerId: null };
  ...
  bindEvents() {
    ...
    this.planetSelect.addEventListener("change", (event) => {
      if (event.target.value) this.focusPlanet(event.target.value);
    });
  ...
  getOrbitDistance(planet) {
    if (this.scaleMode === "visual") return planet.orbit;
    return 70 + Math.log10(planet.au + 1) * 380;
  }

  getPlanetSize(planet) {
    if (this.scaleMode === "visual") return planet.size;
  ...
  toggleCinemaMode() {
    ...
    document.querySelector(".legend-card").style.display = this.cinemaMode
      ? "none"
      : "block";
    if (this.cinemaMode) this.hideInfo();
  }
  ...
  showPlanet(key, selectPlanet) {
    ...
    this.planetDetails.innerHTML = `
          <div class="detail-item"><span>Distance from Sun</span><strong>${planet.distance}</strong></div>
          <div class="detail-item"><span>Orbital Period</span><strong>${planet.period}</strong></div>
          <div class="detail-item"><span>Diameter</span><strong>${planet.diameter}</strong></div>
          <div class="detail-item"><span>Temperature</span><strong>${planet.temperature}</strong></div>
          <div class="detail-item"><span>Known Moons</span><strong>${planet.moons}</strong></div>
          <div class="detail-item"><span>Scale Mode</span><strong>${this.scaleMode === "real" ? "Real-distance inspired" : "Visual presentation"}</strong></div>
        `;
  ...
  focusPlanet(key) {
    ...
    this.cameraTarget.scale = Math.max(this.camera.scale, 1.72);
    this.showPlanet(key, true);
  ...
  zoom(event) {
    ...
    const oldScale = this.cameraTarget.scale;
    ...
    this.cameraTarget.x -= pointerX * (newScale / oldScale - 1);
    this.cameraTarget.y -= pointerY * (newScale / oldScale - 1);
    this.cameraTarget.scale = newScale;
  }
  ...
  updateCamera() {
    ...
    this.stars.style.transform = `translate(${this.camera.x * 0.025}px, ${this.camera.y * 0.025}px)`;
  }

  updateFocusTarget() {
    if (!this.focusTarget || this.drag.active) return;

    const position = this.positions.get(this.focusTarget.dataset.planet);
    if (!position) return;

    this.cameraTarget.x = -position.x * this.cameraTarget.scale;
    this.cameraTarget.y = -position.y * this.cameraTarget.scale;
  }

  updateCameraMotion() {
    const ease = this.focusTarget ? 0.07 : 0.1;
    this.camera.x += (this.cameraTarget.x - this.camera.x) * ease;
    this.camera.y += (this.cameraTarget.y - this.camera.y) * ease;
    this.camera.scale += (this.cameraTarget.scale - this.camera.scale) * ease;
    this.updateCamera();
  }
  ...
  animate(now) {
    ...
    if (this.isPlaying) this.elapsed += delta * this.speed;
